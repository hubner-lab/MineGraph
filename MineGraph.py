...
Pipeline (construct command):
  ...
  Step 4 – run_pggb.py                → rakanhaib/opggb  (default)
         OR pggb directly             → ghcr.io/pangenome/pggb:latest  (--pggb-image)
...
PGGB_OFFICIAL_IMAGE  = "ghcr.io/pangenome/pggb:latest"
...
def _run_pggb_direct(
    ...
    """
    Run pggb directly inside the official PGGB image (Step 4 only).

    Parameters are read from the params.yaml written by Step 3, so Steps 1–3
    must already have completed.  abs_out is mounted as /output inside the
    container, matching the paths stored in params.yaml.

    Steps 1, 2, 3, and 5 always use rakanhaib/opggb — this function is only
    called when the caller explicitly passes --pggb-image.
    """
    params_yaml = Path(abs_out) / "params.yaml"
    try:
        with open(params_yaml) as _f:
            params = yaml.safe_load(_f) or {}
    except Exception as exc:
        print(f"[ERROR] Cannot read {params_yaml}: {exc}", file=sys.stderr)
        return 1
    ...
    if not path_to_input:
        print("[ERROR] params.yaml is missing 'path_to_input' — was Step 3 completed?",
              file=sys.stderr)
        return 1

    if segment_length >= 1000:
        segment_length_str = f"{int(segment_length / 1000)}k"
    else:
        segment_length_str = str(int(segment_length))

    pggb_cmd = [
        "pggb",
        "-i", path_to_input,
        "-o", "/output/pggb_output",
        "-n", str(number_of_haplotypes),
        "-t", str(threads),
        "-p", str(percent_identity),
        "-s", segment_length_str,
        "-m", "-M",
    ]
    if vcf_reference:
        pggb_cmd += ["-V", vcf_reference]
    if pggb_resume:
        pggb_cmd.append("--resume")
...
def run_construct(args, profiler: Optional["BenchmarkProfiler"] = None) -> int:
    ...
    cp.add_argument("--species",     default=None,
                        help="Species/clade name passed to RepeatMasker -species. "
                             "If omitted, RepeatMasker runs without the -species flag "
                             "(uses its own default repeat library). "
                             "Examples: viridiplantae, embryophyta, magnoliophyta, monocotyledons, poaceae.")
    ...
    image      = args.image
    pggb_image = getattr(args, "pggb_image", None)
    ...
    if mode in ("all", "extract-tr", "construct-graph"):
        ...
        if skip_tr:
            ...
            rm_species_flag = f" -species {rm_species}" if rm_species else ""
            rm_cmd = (
                ["docker", "run", "--rm"]
                + _platform_flags()
                + [
                    ...
                    f"-no_is -pa {threads} -s -gff{rm_species_flag}",
                ]
            )
    ...
    # ------------------------------------------------------------------
    # STEP 4 – PGGB graph construction
    #   Default: rakanhaib/opggb (via run_pggb.py wrapper)
    #   Optional: official ghcr.io/pangenome/pggb image (--pggb-image)
    #             pggb is called directly; Steps 1–3 and 5 always use opggb.
    # ------------------------------------------------------------------
    if skip_pggb:
        ...
        if pggb_image:
            print(f"[INFO] Step 4 — using official PGGB image: {pggb_image}")
            rc = _run_pggb_direct(
                pggb_image=pggb_image,
                abs_out=abs_out,
                threads=threads,
                pggb_resume=pggb_resume,
                dry_run=dry_run,
                label="Step 4 — PGGB graph construction (official image)",
                profiler=profiler,
            )
        else:
            pggb_args = [threads]
...
""",
    ...
    parser.add_argument("--image",         default=OPGGB_IMAGE_DEFAULT,
                        help=f"opggb image used for Steps 1, 3, and 5 (default: {OPGGB_IMAGE_DEFAULT})")
    parser.add_argument("--pggb-image",    dest="pggb_image", default=None,
                        metavar="IMAGE",
                        help=f"Use the official PGGB image for Step 4 (graph construction) only.\n"
                             f"When set, pggb is called directly inside this image instead of\n"
                             f"via the run_pggb.py wrapper in opggb.\n"
                             f"Steps 1, 3, and 5 always use --image (opggb).\n"
                             f"Example: --pggb-image {PGGB_OFFICIAL_IMAGE}")

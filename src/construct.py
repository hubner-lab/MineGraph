...
def run_workflow(data_dir, output_dir, metadata_file, threads=16, tree_pars=10, tree_bs=10, quantile=0.25, top_n=50,
                 ...
                 tree_type='graph', softcore_threshold=90, transclose_batch=None, tmpfs_size=None):
    try:
        ...
        if mode in ["all", "extract-tr", "construct-graph"]:
            ...
            prepare_command = [
                                  "docker", "run", "--rm", "--platform", f"{docker_platform}", "-v",
                                  f"{os.path.abspath(data_dir)}:/data",
                                  "-v", f"{os.path.abspath(output_dir)}:/output",
                                  "rakanhaib/opggb", "python", "/src/prepare_and_mash_input.py", "/data", f"{threads}"
                              ] + fasta_files
        ...
        if mode in ["all", "extract-tr", "construct-graph"]:
            ...
            repeatmask_command = [
                "docker", "run", "--rm", "--platform", f"{docker_platform}", "-v",
                f"{os.path.abspath(output_dir)}:/data",
        ...
        # Mount a tmpfs RAM disk inside the container for seqwish's mmap sort files.
        # This keeps all random I/O in RAM instead of on slow EBS.
        # Size is in GB; rule of thumb: ~2x the total uncompressed input size.
        if tmpfs_size is not None:
            pggb_command += ["--tmpfs", f"/tmp:exec,size={tmpfs_size}g"]
        pggb_command += [
            "rakanhaib/opggb", "python", "/src/run_pggb.py", str(threads)
        ]
        if transclose_batch is not None:
            pggb_command += ["--transclose-batch", str(transclose_batch)]
        if tmpfs_size is not None:
            # Direct seqwish temp files to the in-container tmpfs
            pggb_command += ["--seqwish-temp-dir", "/tmp"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="seqwish transitive closure batch size in base pairs (pggb -B, default 10000000). "
             "Increase to reduce disk sort passes. For 709 chloroplast genomes (~106Mbp total), "
             "use 500000000 to process in one pass entirely in RAM."
    )
    parser.add_argument(
        "--tmpfs_size",
        type=int,
        default=None,
        help="Size in GB of a tmpfs RAM disk mounted at /tmp inside the Docker container. "
             "seqwish mmap temp files are redirected there, eliminating EBS random I/O. "
             "Rule of thumb: ~2x total uncompressed input size. Example: 50 for ~25GB input. "
             "Requires sufficient instance RAM. Omit to disable (files land on EBS as before)."
    )
    ...
    run_workflow(
        ...
        transclose_batch=args.transclose_batch,

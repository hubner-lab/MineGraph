...
def rename_and_compress_fasta(fasta_dir, output_file="/output/panSN_output.fasta", file_extension="fasta",
                              selected_files=None):
    """
    Processes selected FASTA files in the directory by renaming headers based on the sanitized original header,
    merging them, compressing, and returning the headers used for later reference selection.
    """
    sequence_count = 0
    headers = []  # To store sanitized headers for future reference selection
    # Check if all selected files exist in the directory
    if selected_files:
        ...
        if missing_files:
            ...
            exit(1)  # Exit if any selected file is not found
    ...
    try:
        ...
        # Process each FASTA file in the directory or only selected ones if provided
        for file_name in os.listdir(fasta_dir):
            if file_name.endswith(f".{file_extension}") and file_name != output_file:
                # Skip if selected_files is defined and file_name is not in it
                ...
                with open(fasta_path, 'r') as infile, open(output_file, 'a') as outfile:
                    # Read the original header and sanitize it
                    ...
                    # Remove leading ">" for sanitization
                    sanitized_header = sanitize_header(original_header[1:])
                    headers.append(sanitized_header)  # Store sanitized header for reference

                    # Write the sanitized header and sequence to the output file
        ...
        # Downsample if needed and compress the final merged FASTA file
        downsampled_fasta = downsample_fasta(output_file, target_bases=3_000_000)
        compressed_output = bgzip_and_index(output_file)

        return compressed_output, downsampled_fasta, sequence_count, headers  # Return sanitized headers

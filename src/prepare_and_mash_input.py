...
def rename_and_compress_fasta(
    ...
    try:
        ...
        for file_name in sorted(os.listdir(fasta_dir)):
            ...
            # Multi-record FASTA support (e.g., mito assemblies with 2–3 circles per file)
            with open(output_file, "a") as outfile:
                for contig_idx, record in enumerate(SeqIO.parse(fasta_path, "fasta")):
                    # Contig name: simple integer index within the file
                    new_id = pansn_header(
                    ...
                    record.id = new_id
                    record.name = new_id
                    record.description = ""  # keep header strict

...
def plot_node_histogram_by_paths(
    ...
    softcore_threshold: int = 90
):
    """
    Plots categorized histogram (Unique, Dispensable, Soft-core, Core)
    using a user-defined percentage for soft-core classification.
    """
    import math
    softcore_floor = min(math.floor((softcore_threshold / 100) * num_paths), num_paths - 1)

    unique = [(c, f) for c, f in node_histogram.items() if c == 1]
    dispensable = [(c, f) for c, f in node_histogram.items() if 2 <= c < softcore_floor]
    softcore = [(c, f) for c, f in node_histogram.items() if softcore_floor <= c < num_paths]
    core = [(c, f) for c, f in node_histogram.items() if c == num_paths]

    def unzip(data): return zip(*data) if data else ([], [])
    u_x, u_y = unzip(unique)
    d_x, d_y = unzip(dispensable)
    s_x, s_y = unzip(softcore)
    c_x, c_y = unzip(core)

    plt.figure(figsize=(12, 8))
    plt.bar(u_x, u_y, color='blue', label='Unique')
    plt.bar(d_x, d_y, color='green', label='Dispensable')
    plt.bar(s_x, s_y, color='orange', label='Soft-core')
    plt.bar(c_x, c_y, color='red', label='Core')

    plt.xlabel("Node count across haplotypes")
    plt.ylabel("# of Nodes (log scale)")
    plt.title(f"Node Category Distribution")
    plt.yscale('log')
    plt.legend()
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    ...
    plt.close()
    print(f"✅ Categorized histogram saved as {output_file}")
...
def extract_graph_stats(
    ...
    """
    Extracts and saves graph statistics, classifying nodes into
    Unique, Dispensable, Soft-core, and Core categories.

    The user can set the soft-core threshold (default 90% of haplotypes).
    """
    import math

    print(f"Extracting and saving graph statistics (soft-core ≥{softcore_threshold:.1f}% rule)...")

    # --- Threshold based on user-defined percentage ---
    softcore_floor = min(math.floor((softcore_threshold / 100) * num_haplotypes), num_haplotypes - 1)
    print(f"Soft-core threshold: ≥{softcore_floor} & <{num_haplotypes} (≥{softcore_threshold:.1f}% of haplotypes)")

    # --- Classification ---
    unique_nodes = c_dist.get(1, 0)
    dispensable_nodes = sum(v for k, v in c_dist.items() if 2 <= k < softcore_floor)
    softcore_nodes = sum(v for k, v in c_dist.items() if softcore_floor <= k < num_haplotypes)
    core_nodes = c_dist.get(num_haplotypes, 0)

    graph_stats = {
        "Statistic": [
            "Total number of nodes",
            "Unique nodes (count=1)",
            f"Dispensable nodes (2 ≤ count < {softcore_floor})",
            f"Soft-core nodes (≥{softcore_floor} & < {num_haplotypes})",
            f"Core nodes (= {num_haplotypes})",
        ...
        "Explanation": [
            ...
            "Nodes restricted to a single haplotype (unique/private regions).",
            f"Nodes shared by multiple but fewer than {softcore_floor} haplotypes — variable (dispensable) regions.",
            f"Nodes present in ≥{softcore_floor} but <{num_haplotypes} haplotypes — soft-core regions conserved across ≥{softcore_threshold:.1f}% of haplotypes.",
            "Nodes found in all haplotypes (core regions).",
            "Total number of edges linking nodes.",
            "Mean number of edges per node (average degree).",
            "Graph density (|E| / |V| choose 2).",
            "Total number of haplotype paths represented in the graph.",
            "Genome-wide nucleotide diversity (π) estimated from the VCF file."
        ]
    }

    lengths = {k: len(v) for k, v in graph_stats.items()}
    if len(set(lengths.values())) != 1:
        raise ValueError(f"graph_stats length mismatch: {lengths}")

    df_stats = pd.DataFrame(graph_stats)
    df_stats.to_csv(f"{name}_stats.csv", index=False)
    print(f"✅ Graph statistics saved to {name}_stats.csv")

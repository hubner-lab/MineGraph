...
def generate_graph_based_tree(
    odgi_matrix_tsv,
    output_prefix="graph_phylo_tree_sets"
):
    print(f"[graph-tree] Reading ODGI matrix as presence sets: {odgi_matrix_tsv}")
    paths, node_sets, _ = read_odgi_as_presence_sets(odgi_matrix_tsv)

    print(f"[graph-tree] Computing Jaccard distance matrix for {len(paths)} paths...")
    D = compute_jaccard_distance_matrix(node_sets)
    ...
    print("[graph-tree] Converting to Newick...")
    newick = _linkage_to_newick(Z, labels=paths)

    tree_file = f"{output_prefix}.tree"
    tree_dir2 = os.path.join("/data","MineGraph_output", "phylogenetics_msa",tree_file)
    with open(tree_dir2, "w") as f:
        f.write(newick)
